...
@RestController
@RequestMapping("/trips/{tripId}/packing-list")
...
public class PackingListController {
    ...
    public PackingDtos.PackingListRes assign(
            @PathVariable Long tripId,
            @Valid @RequestBody PackingDtos.AssignTemplatesReq req) {

        Trip trip = tripService.get(tripId);
        List<TripPackingItem> items = packing.assignTemplates(trip, req);
        return new PackingDtos.PackingListRes(items.stream()
                .map(i -> new PackingDtos.PackingItemRes(i.getId(), i.getName(), i.getQuantity(), i.isChecked()))
                .toList());
    }

    @GetMapping
    public PackingDtos.PackingListRes list(@PathVariable Long tripId) {
        List<TripPackingItem> items = tripItems.findByTripIdOrderByNameAsc(tripId);
        return new PackingDtos.PackingListRes(items.stream()
                .map(i -> new PackingDtos.PackingItemRes(i.getId(), i.getName(), i.getQuantity(), i.isChecked()))
                .toList());
    }
